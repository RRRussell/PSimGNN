...
class MergedGraphData(PyGSingleGraphData):  # Yunsheng Bai renamed both classes
    r"""A plain old python object modeling a batch of graphs as one big
    (dicconnected) graph. With :class:`torch_geometric.data.Data` being the
    base class, all its methods can also be used here.

    Yunsheng Bai: Merge + split for efficient (merge) and flexible (split)
    ...
    def from_data_list(data_list, metadata_list=None):  # merge
        ...
        Yunsheng Bai: Keep track of and return the starting
        ...
        yba_indices_list = []  # Yunsheng Bai added this line

        cumsum = 0
        for i, data in enumerate(data_list):
            num_nodes = data.num_nodes
            batch.batch.append(torch.full((num_nodes,), i, dtype=torch.long,
                                          device=FLAGS.device))  # yba
            for key in data.keys:
                item = data[key]
                # item = item + cumsum if data.__cumsum__(key, item) else item
                batch[key].append(item)

            # Yunsheng Bai added the following line.
            yba_indices_list.append((cumsum, cumsum + num_nodes))

            cumsum += num_nodes
        # print(cumsum)
        ###################################
        # Derek added the following lines
        ...
        # print("batch.con",batch.contiguous())
        # print("yba_ind",yba_indices_list)
        # Yunsheng Bai modified the following line.
        return {'merge': batch.contiguous(), 'ind_list': yba_indices_list}
    ...
    def to_data_list(batch_data, tensor):  # split
        r"""
        Yunsheng Bai: Extract individual graph node embeddings from the
